...
load_data <- function(csv){
	...
	for (s in seq(Strains)){
  		...
  		invisible(suppressMessages({(lapply(seq(nrow(df)), function(x) {
  			tmp[[x]] <<- reshape2::melt(df[df$NetworkFilename == unique(df$NetworkFilename)[x], which(names(df) %in% c('MouseID','Sex','Strain',sapply(seq(60), function(x) paste0("minuteMax",x))))])}))}))
